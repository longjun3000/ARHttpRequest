/**
 * @file ARHttpRequestSessionAdapter.m
...
@property (nonatomic, strong) NSOperationQueue *defaultQueue; //默认队列
@property (nonatomic) BOOL isUseDefaultQueue; //是否使用默认队列

@property (nonatomic, strong) NSMutableDictionary *taskDict; //缓存会话任务的相关参数
@property (nonatomic, strong) NSURLSession *defaultSession; //默认会话
@property (assign) int taskCount; //会话任务总数
...
- (void)dealloc
{
    ...
    if (_defaultQueue) {
        ...
        [_defaultQueue removeObserver:self forKeyPath:@"operations"];
    }
...
{
    if (_defaultQueue) {
         NSLog(@">>> queueStart, queue.operations.count=%lu",(unsigned long)self.defaultQueue.operations.count);
...
- (void)startSessionTask:(NSURLSessionTask*)task
{
    if (!_isUseDefaultQueue) { //如果不使用队列，则立即执行任务，否则由队列来管理执行与否
        [task resume]; //执行Session任务
    }
    else {
        // 创建一个操作
        ...
        // 如果队列有操作且队列并发数为1，则设置操作顺序执行
        if (_defaultQueue && _defaultQueue.maxConcurrentOperationCount == 1
            && _defaultQueue.operations && _defaultQueue.operations.count > 0) {
            NSOperation *lastOperation = [_defaultQueue.operations lastObject];
            [operation addDependency:lastOperation]; //新操作依赖上一个操作
        }
        // 新操作添加到队列
       ...
       failedBlock:(ARNetFailedBlock _Nonnull)failedBlock
{
    ...
    // 创建一个上传任务
    NSURLSessionUploadTask *uploadTask =[self.defaultSession uploadTaskWithRequest:request fromData:fileData completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        // 当前任务回调Block
        if (error) {
            failedBlock(error, userInfo);
        }
...
//                           , @"downloadSession": session
    ...
    // 执行任务
    [self startSessionTask:downloadTask];
}
...
//        NSURLSession *session = _taskDict[keyName][@"downloadSession"];
        ...
        if (_defaultSession && resumeData) {
            NSURLSessionDownloadTask *downloadTask = [_defaultSession downloadTaskWithResumeData:resumeData];
