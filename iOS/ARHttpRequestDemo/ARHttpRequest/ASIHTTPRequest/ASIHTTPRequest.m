...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
        ...
        ASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"The request timed out",@""),NSLocalizedDescriptionKey,nil]];
        ASIAuthenticationError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"Authentication needed",@""),NSLocalizedDescriptionKey,nil]];
        ASIRequestCancelledError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"The request was cancelled",@""),NSLocalizedDescriptionKey,nil]];
        ASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"Unable to create request (bad url?)",@""),NSLocalizedDescriptionKey,nil]];
        ASITooMuchRedirectionError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"The request failed because it redirected too many times",@""),NSLocalizedDescriptionKey,nil]];
...
- (void)startRequest
{
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {       
        ...
        if (![self validatesSecureCertificate]) {
            ...
            //                                      nil];
            NSDictionary *sslProperties = [[NSDictionary alloc] initWithObjectsAndKeys:@NO, kCFStreamSSLValidatesCertificateChain, kCFNull, kCFStreamSSLPeerName, nil];
...
- (void)updateUploadProgress
{
    ...
    if(progressBlock){
		...
		[self performBlockOnMainThread:^{ if (progressBlock) { progressBlock(value, totalSize); }}];
	}
	#endif
}
...
-(void)removeUploadProgressSoFar
{
    ...
    if(progressBlock){
		unsigned long long totalSize = [self postLength];
		[self performBlockOnMainThread:^{  if (progressBlock) { progressBlock(progressToRemove, totalSize); }}];
	}
	#endif
}
...
- (ASIBasicBlock)didCompletionBlock {
    return completionBlock;
}

- (ASIBasicBlock)didFailedBlock {
    return failureBlock;
}
...
- (void)setprogressBlock:(ASIProgressBlock)aprogressBlock
{
	[progressBlock release];
